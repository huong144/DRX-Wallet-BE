...
import TrxGateway from '../src/TrxGateway';
import { expect, assert } from 'chai';
import { TransactionStatus } from 'sota-common';

describe('TrxGateway::test-create-account', () => {
  const trx = new TrxGateway();
  it('Create account', async () => {
    const account = await trx.createAccountAsync();
    assert.exists(account);
  });
});

describe('TrxGateway::test-get-one-transaction', () => {
  const trx = new TrxGateway();
  it('Return transaction info if txid valid', async () => {
    const tx = await trx.getOneTransaction('0x2de03b4e82e3869402677c14d497e48dc7065819c4b69ae9531bafad6f4c73ed');
  ...
  it('Return null if unknown transaction', async () => {
    const tx = await trx.getOneTransaction('0x1111111111111111111111111111111111111111111111111111111111111111');
  ...
  it('Throw error if txid invalid', async () => {
    let error;
    try {
      const tx = await trx.getOneTransaction('abc');
    } catch (err) {
      error = err;
    }
...
describe('TrxGateway::test-get-transaction-by-ids', () => {
  const trx = new TrxGateway();
  it('Return transaction array with txids valid', async () => {
    ...
    const txs = await trx.getTransactionsByIds(txids);
...
describe('TrxGateway::test-get-one-block', () => {
  const trx = new TrxGateway();
  it('Return block info if block number valid', async () => {
    const block = await trx.getOneBlock(3590100);
    assert.exists(block);
  });

  it('Return null if block number not exist', async () => {
    const block = await trx.getOneBlock(10000000000);
  ...
  it('Throw error if block number invalid', async () => {
    let error;
    try {
      const block = await trx.getOneBlock('abc');
    } catch (err) {
      error = err;
    }
    expect(error).to.exist.and.be.instanceof(Error);
...
describe('TrxGateway::test-get-block-count', () => {
  const trx = new TrxGateway();
  it('Return block count', async () => {
    const count = await trx.getBlockCount();
    assert(count > 0);
  });
});

describe('TrxGateway::test-get-block-transactions', () => {
  const trx = new TrxGateway();
  it('Return txs of block', async () => {
    const txs = await trx.getBlockTransactions(3590100);
    assert(txs.length > 0);
  });
  ...
  it('Throw error if block not exist', async () => {
    let error;
    try {
      const txs = await trx.getBlockTransactions(100000000);
    } catch (err) {
      error = err;
    }
    expect(error).to.exist.and.be.instanceof(Error);
  ...
  it('Throw error if block number invalid', async () => {
    let error;
    try {
      const txs = await trx.getBlockTransactions('abc');
    } catch (err) {
      error = err;
    }
    expect(error).to.exist.and.be.instanceof(Error);
...
describe('TrxGateway::test-get-multi-block-transactions', () => {
  const trx = new TrxGateway();
  it('Return txs from block a to block b', async () => {
    const txs = await trx.getMultiBlocksTransactions(3590100, 3590105);
    assert.exists(txs);
  });
  ...
  it('Throw error if block start less than block end', async () => {
    let error;
    try {
      const txs = await trx.getMultiBlocksTransactions(3590105, 3590100);
    } catch (err) {
      error = err;
    }
...
describe('TrxGateway::test-get-address-balance', () => {
  const trx = new TrxGateway();
  it('Return balance of account', async () => {
    const balance = await trx.getAddressBalance('0xe8132179d9181da72109CD65BE6ad9B4F2a99A8B');
  ...
  it('Throw error if address invalid', async () => {
    let error;
    try {
      const balance = await trx.getAddressBalance('111111111111111111111111111111111111111111');
    } catch (err) {
      error = err;
    }
    expect(error).to.exist.and.be.instanceof(Error);
...
describe('TrxGateway::test-get-transaction-status', () => {
  const trx = new TrxGateway();
  it('Return UNKNOWN if txid invalid', async () => {
    const result = await trx.getTransactionStatus('0x1111111111111111111111111111111111111111111111111111111111111111');
    assert.equal(result, TransactionStatus.UNKNOWN);
  });

  it('Return COMPLETED if txid valid and enought confirmation', async () => {
    const result = await trx.getTransactionStatus('0x2de03b4e82e3869402677c14d497e48dc7065819c4b69ae9531bafad6f4c73ed');
    assert.equal(result, TransactionStatus.COMPLETED);
  });
});

describe('TrxGateway::createRawTransaction', () => {
  const gateway = new TrxGateway();
...
describe('TrxGateway::test-sign-raw-tx-by-singer-private-key', () => {
  const gateway = new TrxGateway();
...
describe('TrxGateway::test-send-raw-transaction', () => {
  const gateway = new TrxGateway();
